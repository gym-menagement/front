...
export default class PaymenttypeModel {
  ...
  static async patch(id: number, item: Partial<Paymenttype>) {
    const res = await patch<Paymenttype>(`/paymenttype/${id}`, item);
    return res.data;
  }
  ...
  static async findpage(params?: PaymenttypeSearchParams) {
    const res = await get<ApiResponse<Paymenttype>>('/paymenttype', { params });
    return res.data
  }
