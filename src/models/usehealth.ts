...
export default class UsehealthModel {
  ...
  static async patch(id: number, item: Partial<Usehealth>) {
    const res = await patch<Usehealth>(`/usehealth/${id}`, item);
    return res.data;
  }
  ...
  static async findpage(params?: UsehealthSearchParams) {
    const res = await get<ApiResponse<Usehealth>>('/usehealth', { params });
    return res.data
  }
